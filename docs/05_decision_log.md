...
- AIが設計意図を理解できるようにする
- 同じ議論を繰り返さない
- GenesisPrediction 専用の厳格ルールを汎用AI用ログと分離して保持する
...
## Decision: GenesisPrediction and General AI decision logs must be separated

背景

Open WebUI 上で GenesisPrediction 用の厳格ルールと、汎用AI 用の思考支援ルールを同時運用し始めた。

しかし両者は目的が異なる。

- GenesisPrediction = 分析・予測システム
- General AI = 思考支援・意思決定支援

同一の decision_log で管理すると、

- 汎用AI の柔軟ルールが GenesisPrediction の厳格ルールに混入する
- GenesisPrediction 固有ルールが汎用AI に過剰適用される
- RAG 検索時にノイズが混ざる
...
GenesisPrediction 用 decision_log は、汎用AI 用 decision_log から完全分離する。

固定ルール

```text
GenesisPrediction rules must live only in GenesisPrediction decision_log
General AI rules must live only in general decision log
Cross-reference is allowed, mixed storage is not allowed
```
...
## Decision: UI Design Principle (Critical)
...
```text
UI is display-only.
UI must NOT perform calculations, contain business logic,
determine values, or translate text.
All logic must exist in the analysis layer.
```

意図

- RAG 検索時に「UI設計」「原則」「UI design principle」で確実にヒットさせる
- 汎用UI論ではなく GenesisPrediction 固有原則を常に優先させる
- Open WebUI Knowledge 参照時の取り違えを防ぐ

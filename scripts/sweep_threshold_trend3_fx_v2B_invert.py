# scripts/sweep_threshold_trend3_fx_v2B_invert.py
# Sweep threshold for Trend3 FX model with EXPECTED SIGN INVERTED (v2B).
#
# Uses the SAME loaders/logic as backtest_trend3_fx_v2.py (v2A) via dynamic import,
...
#
# Output:
#   analysis/prediction_backtests/threshold_sweep_trend3_fx_v2B_invert_YYYYMMDD_HHMMSS.{txt,csv,json}
#
# Run:
#   .\.venv\Scripts\python.exe scripts\sweep_threshold_trend3_fx_v2B_invert.py
#   .\.venv\Scripts\python.exe scripts\sweep_threshold_trend3_fx_v2B_invert.py --tmin 0.02 --tmax 0.20 --tstep 0.01 --min-calls 5
#   .\.venv\Scripts\python.exe scripts\sweep_threshold_trend3_fx_v2B_invert.py --max-uncertainty 0.25
#
...
def _load_v2a_module() -> Any:
    ...
    modname = "backtest_trend3_fx_v2A_mod_for_v2B_threshold_sweep"
...
def _write_outputs(df: pd.DataFrame, meta: Dict[str, Any]) -> Dict[str, str]:
    ...
    ts = _now_ts()
    base = OUT_DIR / f"threshold_sweep_trend3_fx_v2B_invert_{ts}"
    ...
    lines: List[str] = []
    lines.append("Threshold sweep: Trend3 FX (v2B invert expected_fx_sign)")
...
def main() -> int:
    ...
    fx, fx_dbg = v2a.load_fx_thb_per_jpy()
    ...
    meta = {
        ...
        "notes": "Expected fx sign is inverted (multiplied by -1).",
    }
